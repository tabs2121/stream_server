...
public:
    ...
    // bidirektional: Callback für empfangene Daten
    std::function<void(const uint8_t* data, size_t len, std::string client_id)> on_data_received;

    // bidirektional: Sendehilfen
    void send_to_all(const uint8_t* data, size_t len);
    void send_to_client(const std::string& client_id, const uint8_t* data, size_t len);
...
protected:
    ...
    struct Client {
        ...
        uint8_t offset = 0;
        uint8_t buffer[260]; // Empfangspuffer
        std::vector<uint8_t> send_buffer; // Sende-Puffer
